...
const icsAPI = (
  ...
  start_date.setMinutes(
    start_date.getMinutes() - start_date.getTimezoneOffset() - 8 * 60
  );
  ...
  for (let index = 0; index < courseList.length; index++) {
    ...
    for (let i = 0; i < course.index.meetings.length; i++) {
      ...
      let date_start = addMinutes(
        date_ptr,
        parseInt(cur_meet.time_start.slice(0, 2)) * 60 +
          parseInt(cur_meet.time_start.slice(2, 4))
      );

      console.log(date_start);
      let date_end = addMinutes(
        date_ptr,
        parseInt(cur_meet.time_end.slice(0, 2)) * 60 +
          parseInt(cur_meet.time_end.slice(2, 4))
      );
      ...
      for (let j = 1; j <= 13; j++) {
        ...
        events.push({
          ...
          startOutputType: "utc",
            ...
            date_end.getUTCDate(),
            date_end.getUTCHours(),
            date_end.getUTCMinutes(),
          ],
          endInputType: "utc",
          endOutputType: "utc",
        });
